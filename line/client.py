...
class LineClient(LineAPI):
    ...
    def __init__(self, email=None, password=None, authToken=None, app_name=None, com_name="Line_Carpedm"):
        ...
        self._headers['X-Line-Application'] = app_name
        self._headers['X-Line-Carrier']     = self.carrier
        self._headers["x-lal"]              = "in_ID"
        
        if not (authToken or id and password):
            msg = "Id and password or authToken is nothing !"
            print(msg)
            self.qrLogin(APP_NAME=app_name)
        ...
        if (email and password):
            ...
            self.login(systemName=com_name)
            self.ready()

        self.revision = self._getLastOpRevision()
        self.getProfile()
        print("=====[ Line Profile ]=====\nName : {}\nMid : {}\nAuthtoken : {}\n# Sukses Login #".format(self.getProfile().name, self.getProfile().id, self._headers['X-Line-Access']))
    ...
    @check_auth
    def sendText(self, mid, message, seq=0):
        msg = Message()
        msg.to = mid
        msg.text = message
        return self._sendMessage(msg, seq)
